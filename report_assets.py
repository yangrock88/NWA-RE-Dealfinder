"""Dashboard presentation assets for report.py.

Kept in their own module so report.py stays focused on HTML assembly.
CSS holds the design tokens + component styles; JS holds the client-side
filtering logic. Both are plain strings injected verbatim into the
generated single-file dashboard.
"""
...
(function(){
  ...
  window.quickFilter = function(action){
    ...
    if (!wasActive) {
      ...
      else if(action==='below'){ $id('f-qmax').value=Math.round(MED*(window.NWA_RATIO||0.82)||165); }
